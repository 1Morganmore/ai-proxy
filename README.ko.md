...
업스트림은 라우팅 워크플로우이므로, 주제와 무관한 질문에는 HTTP 200 + 고정 40자 거절 문구(`您好，您的提问超出了我的回答范畴…`)가 반환됩니다. 이 프록시는 이를 감지해 비스트리밍 요청은 `503` + `{"detail": "fanzha_out_of_domain"}`로, 스트리밍 요청은 `code: 503` 오류 이벤트 + `[DONE]`로 바꾸므로 호출 측 폴백 체인이 정중하게 막힌 답변을 정상 응답으로 오해하지 않습니다. `FANZHA_REFUSAL_AS_ERROR=false`로 원문 전달을 되살릴 수 있습니다.

답변 끝의 상용구 처리는 `STRIP_BOILERPLATE`로 조절합니다: `off`(원문), `intent-only`(기본값, 라우터가 답변에 섞어 내보내는 `current_intent:` 블록 제거), `full`(끝에 붙는 `防范建议`/`温馨提示` 류의 닫구 제거). 스트리밍과 비스트리밍은 동일한 줄 단위 필터를 거치므로 같은 업스트림 답변은 어느 쪽에서든 동일한 텍스트가 됩니다.

지식베이스 조회는 채트 게이트를 거치지 않는 별도 경유 라우트입니다: `GET /kb/search?keyword=…&locale=zh-CN`(업스트림 `/api/v1/anti-fraud/search`) 및 `GET /kb/term?keyword=…&limit=10`(업스트림 `/api/v1/faq/search`). 두 라우트는 설정된 토큰으로 서버 측에서 호출하며 업스트림 JSON과 HTTP 상태를 그대로 전달하고, 루프백 외의 피어에서는 `403`으로 거부됩니다.
